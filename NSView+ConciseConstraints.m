//
//  NSView+ConciseConstraints.m
//
//  Created by Jakob Egger on 12/12/13.
//

#import "NSView+ConciseConstraints.h"

@implementation NSView (ConciseConstraints)
...
-(void)CC_addConstraintFormat:(NSString*)formatString, ... {
	...
	while (*fmt) {
		if  (*fmt=='%') {
			...
			switch(*fmt) {
				...
				case 'v':
				{
					NSView *view = va_arg(args, NSView*);
					NSString *viewName = [view className];
					int i=1;
					while (views[viewName]) {
						i++;
						viewName = [NSString stringWithFormat:@"%@%d", [view className], i];
					}
					views[viewName] = view;
	...
	[self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:constraintString options:nil metrics:nil views:views]];
}
